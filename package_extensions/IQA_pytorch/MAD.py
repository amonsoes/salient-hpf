...
def hi_index(ref_img, dst_img):
    ...
    x = torch.view_as_real(torch.fft.fft(ref, dim=2)) # onsided=False is included in fft.fft
    x1 = batch_fftshift2d(x)
    x2 = batch_ifftshift2d( x1 * csf )
    #ref = real(torch.ifft(x2,2))
    ref = real(torch.view_as_real(torch.fft.ifft(torch.view_as_complex(x2), dim=2)))
    #x = torch.rfft(dst, 2, onesided=False)
    x = torch.view_as_real(torch.fft.fft(dst, dim=2))
    x1 = batch_fftshift2d(x)
    x2 = batch_ifftshift2d( x1 * csf )
    #dst = real(torch.ifft(x2,2))
    dst = real(torch.view_as_real(torch.fft.ifft(torch.view_as_complex(x2), dim=2)))
...
def gaborconvolve(im):
    ...
    imagefft = torch.view_as_real(torch.fft.fft(im, dim=2))
    ...
    for o in range(norient):                   # For each orientation.
        ...
        for s in range(nscale):        # For each scale.
            ...
            # c =  imagefft * filter
            #e0 = torch.ifft( imagefft * filter, 2 ) # TODO
            e0 = torch.view_as_real(torch.fft.ifft(torch.view_as_complex(imagefft * filter), dim=2))
            E0[o].append(e0)
